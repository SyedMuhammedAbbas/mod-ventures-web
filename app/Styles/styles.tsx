export const Width =
  "flex justify-center w-[320px] sm:w-[500px] md:w-[600px] lg:w-[750px]";

export const FloorDimensions =
  "flex justify-center items-center w-[320px] sm:w-[500px] md:w-[600px] lg:w-[750px] h-[280px] sm:h-[460px] md:h-[560px] lg:h-[710px]";

export const CloudsDimensions =
  "w-[100px] md:w-[130px] lg:w-[200px] xl:w-[220px]";

export const SmallCloudDimensions =
  "w-[60px] md:w-[70px] lg:w-[80px] xl:w-[110px]";

export const sectionHeadText =
  "text-black md:text-[35px] sm:text-[30px] text-[25px]";
export const sectionSubText =
  "sm:text-[18px] text-[14px] text-secondary uppercase tracking-wider";

export const padding = "sm:px-16 px-6 sm:py-16 py-10";
